 ...
 * programmers.dp[i][j] = i번째부터 j번째까지의 최소가치
 */
public class BOJ_11066_파일합치기 {
    ...
    private static int solve(int start, int end){
        ...
        // 현재값과 바로 다음값의 합 ex) programmers.dp[1][2],programmers.dp[2][3]
        if(end-start == 1)
        ...
        for(int i=start;i<=end;i++){
            //programmers.dp[start][i] + programmers.dp[i+1][end]까지의 합을 구하고 start ~ end 까지의 합(sum[end] - sum[start-1])을 구해서 더하기
